...
public class ElevatorSubsystemTests {
    ...
    private static Stream<Arguments> speedCalculationsTestCases() {
        return Stream.of(
                ...
                Arguments.of(ElevatorConstants.ELEVATOR_BOTTOM_POSITION, ElevatorConstants.ELEVATOR_BOTTOM_POSITION, 0.0),
                // we are in the middle and need to the bottom so move down (positive motor speed)
                Arguments.of(ElevatorConstants.ELEVATOR_L1, ElevatorConstants.ELEVATOR_BOTTOM_POSITION, ElevatorConstants.ELEVATOR_MOTOR_SPEED_DOWN),
                // we are in the middle and need to the bottom so move to the l1 (positive motor speed)
                Arguments.of(ElevatorConstants.ELEVATOR_L3, ElevatorConstants.ELEVATOR_L1, ElevatorConstants.ELEVATOR_MOTOR_SPEED_DOWN),
                // we are at the bottom and need to move up to the first rung so move up (negative motor speed)
                Arguments.of(ElevatorConstants.ELEVATOR_BOTTOM_POSITION, ElevatorConstants.ELEVATOR_L1, ElevatorConstants.ELEVATOR_MOTOR_SPEED_UP),
                // we are at the first postion and need to move up to the highest run so move up (negative motor speed)
                Arguments.of(ElevatorConstants.ELEVATOR_L1, ElevatorConstants.ELEVATOR_L3, ElevatorConstants.ELEVATOR_MOTOR_SPEED_UP),
                // we are at close to the l3 position so don't move
                Arguments.of(ElevatorConstants.ELEVATOR_L3-1, ElevatorConstants.ELEVATOR_L3, 0)
              );
    }
}
